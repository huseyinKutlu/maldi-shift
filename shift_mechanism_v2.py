...
7) invariant_peaks
   - Diagnostic/oracle version: finds m/z bins whose AMR association has consistent sign across sites.

7b) invariant_peaks_target_blind
   - Excludes target labels from feature selection.
   - Selects stable AMR bins only from eligible non-target sites.

7c) source_defined_clusters
   - Fits spectral clusters on DRIAMS-A only, then assigns target isolates.
   - Evaluates cluster-conditioned local experts without target-defined clusters.

7d) invariant_density_weight
   - Combines target-blind invariant features with target-unlabeled density-ratio source weighting.
...
# ---------------------------------------------------------------------
# 7b) Target-blind invariant peak selection
# ---------------------------------------------------------------------

def analysis_invariant_peaks_target_blind(
    lab, mdir, root, species, drug, target, out,
    min_per_class=20, topk=100, seed=0
):
    """
    Select AMR-associated bins without using target labels.

    Selection sites:
        all eligible sites except target

    Source classifier:
        DRIAMS-A only, preserving the external A -> target question.
    ...
    if "DRIAMS-A" not in site_data:
        raise ValueError("DRIAMS-A lacks enough observations for target-blind selection.")
    if len(effects) < 2:
        raise ValueError(
            "Target-blind invariant selection needs at least two eligible non-target sites "
            f"with >= {min_per_class} observations in each class."
        )

    tgt = load_task(lab, mdir, root, species, drug, target)
    if tgt is None or len(np.unique(tgt["y"])) < 2:
        raise ValueError("Target task unavailable or single-class.")

    sites = list(effects)
    E = np.vstack([effects[s] for s in sites])
    signs = np.sign(E)
    ...
    score = np.where(consistent, med_abs, -np.inf)
    rank = np.argsort(score)[::-1]
    rank = rank[np.isfinite(score[rank])]
    selected = rank[:min(topk, len(rank))]
    ...
    peak_df.sort_values(
        ["selected", "median_abs_smd"], ascending=[False, False]
    ).to_csv(out / f"invariant_peaks_target_blind__{target}.csv", index=False)

    src = site_data["DRIAMS-A"]
    rows = []

    _, _, m0 = train_eval_source_target(
        src["X"], src["y"], tgt["X"], tgt["y"], seed=seed
    )
    rows.append(dict(
        feature_set="all_6000",
        selection_protocol="target_blind",
        selection_sites="|".join(sites),
        n_features=src["X"].shape[1],
        **m0,
    ))

    if len(selected) >= 2:
        _, _, m1 = train_eval_source_target(
            src["X"][:, selected], src["y"],
            tgt["X"][:, selected], tgt["y"],
            seed=seed
        )
        rows.append(dict(
            feature_set="target_blind_invariant",
            selection_protocol="target_blind",
            selection_sites="|".join(sites),
            n_features=len(selected),
            **m1,
    ...
    df = pd.DataFrame(rows)
    df.to_csv(
        out / f"invariant_peak_target_blind_performance__{target}.csv",
        index=False
    )
    return df, peak_df, selected


# ---------------------------------------------------------------------
# 7c) Source-defined spectral clusters + local experts
# ---------------------------------------------------------------------

def analysis_source_defined_clusters(
    lab, mdir, root, species, drug, target, out,
    ncomp=100, n_clusters=8, min_source=80, min_target=30,
    min_pos_source=10, seed=0
):
    """
    Fit spectral clusters on DRIAMS-A only and assign target isolates
    using KMeans.predict(). Target data do not define the clusters.

    Outputs:
      - cluster composition shift
      - resistance rate by source-defined cluster
      - cluster-specific source-only local experts
      - routed mixture-of-experts prediction over the full target set
    """
    src = load_task(lab, mdir, root, species, drug, "DRIAMS-A")
    tgt = load_task(lab, mdir, root, species, drug, target)
    if src is None or tgt is None:
    ...
    Zss = scaler.transform(Zs)
    Zts = scaler.transform(Zt)
    ...
    comp = pd.DataFrame(comp_rows)
    comp.to_csv(
        out / f"source_defined_cluster_composition__{target}.csv", index=False
    )
    ...
    pb /= pb.sum()
    mm = 0.5 * (pa + pb)
    js = 0.5 * np.sum(pa * np.log(pa / mm)) + 0.5 * np.sum(pb * np.log(pb / mm))

    global_model = lgb.LGBMClassifier(**CFG, random_state=seed).fit(
        src["X"], src["y"]
    )
    p_global = global_model.predict_proba(tgt["X"])[:, 1]
    global_met = metrics(tgt["y"], p_global)

    p_routed = np.full(len(tgt["y"]), np.nan, dtype=float)
    local_rows = []
    ...
    for k in range(n_clusters):
        ...
        row = dict(
            ...
            n_source=len(ia),
            n_target=len(ib),
            source_prevalence=float(src["y"][ia].mean()) if len(ia) else np.nan,
            target_prevalence=float(tgt["y"][ib].mean()) if len(ib) else np.nan,
        )

        eligible = (
            len(ia) >= min_source
            and len(ib) >= min_target
            and len(np.unique(src["y"][ia])) == 2
            and int((src["y"][ia] == 1).sum()) >= min_pos_source
        )

        if eligible:
            mdl = lgb.LGBMClassifier(**CFG, random_state=seed + k)
            mdl.fit(src["X"][ia], src["y"][ia])
            pk = mdl.predict_proba(tgt["X"][ib])[:, 1]
            p_routed[ib] = pk
            met = metrics(tgt["y"][ib], pk)
            row.update({
                "expert_used": True,
                **{f"local_{kk}": vv for kk, vv in met.items()},
            })
        else:
            if len(ib):
                p_routed[ib] = p_global[ib]
            row["expert_used"] = False

        if len(ib) and len(np.unique(tgt["y"][ib])) == 2:
            gm = metrics(tgt["y"][ib], p_global[ib])
            row.update({
                **{f"global_{kk}": vv for kk, vv in gm.items()},
            })
    ...
    local_df.to_csv(
        out / f"source_defined_cluster_experts__{target}.csv", index=False
    )

    valid = np.isfinite(p_routed)
    routed_met = metrics(tgt["y"][valid], p_routed[valid])

    summary = pd.DataFrame([
        dict(
            method="global_source_model",
            n_clusters=n_clusters,
            js_divergence=js,
            **global_met,
        ),
        dict(
            method="source_defined_routed_experts",
            n_clusters=n_clusters,
            js_divergence=js,
            **routed_met,
        ),
    ...
    summary.to_csv(
        out / f"source_defined_cluster_summary__{target}.csv", index=False
    )
    ...
    assign.to_csv(
        out / f"source_defined_cluster_assignments__{target}.csv", index=False
    )

    return summary, comp, local_df


# ---------------------------------------------------------------------
# 7d) Target-blind invariant peaks + density-ratio weighting
# ---------------------------------------------------------------------

def analysis_invariant_density_weight(
    lab, mdir, root, species, drug, target, out,
    min_per_class=20, topk=100, ncomp=50,
    clips=(2, 5, 10, 20), seed=0
):
    """
    Combine:
      1) target-blind invariant feature selection
      2) target-unlabeled density-ratio weighting

    Target labels are used only for final external evaluation.
    """
    _, peak_df, selected = analysis_invariant_peaks_target_blind(
        lab, mdir, root, species, drug, target, out,
        min_per_class=min_per_class, topk=topk, seed=seed
    )

    if len(selected) < 2:
        raise ValueError("Too few target-blind invariant peaks selected.")

    src = load_task(lab, mdir, root, species, drug, "DRIAMS-A")
    tgt = load_task(lab, mdir, root, species, drug, target)

    Xs_sel = src["X"][:, selected].astype(np.float32)
    Xt_sel = tgt["X"][:, selected].astype(np.float32)

    if Xs_sel.shape[1] > 10:
        nc = min(ncomp, Xs_sel.shape[1] - 1, len(Xs_sel) - 1)
        mu, P = fit_source_svd(Xs_sel, ncomp=max(2, nc), seed=seed)
        Zs = transform_svd(Xs_sel, mu, P)
        Zt = transform_svd(Xt_sel, mu, P)
    else:
        Zs, Zt = Xs_sel, Xt_sel
    ...
    rows.append(dict(
        method="all_6000_unweighted",
        clip=np.nan,
        n_features=src["X"].shape[1],
        domain_auroc_selected_space=dauc,
        effective_sample_size=np.nan,
        **m0,
    ))

    _, _, m1 = train_eval_source_target(
        Xs_sel, src["y"], Xt_sel, tgt["y"], seed=seed
    )
    rows.append(dict(
        method="target_blind_invariant_unweighted",
        clip=np.nan,
        n_features=len(selected),
        domain_auroc_selected_space=dauc,
        effective_sample_size=float(len(src["y"])),
        **m1,
    ))
    ...
    for cl in clips:
        w = density_weights(qs, len(Zs), len(Zt), clip=cl)
        _, _, met = train_eval_source_target(
            Xs_sel, src["y"], Xt_sel, tgt["y"],
            seed=seed, sample_weight=w
        )
        ess = (w.sum() ** 2) / np.sum(w ** 2)
        rows.append(dict(
            method="target_blind_invariant_density_weight",
            clip=float(cl),
            n_features=len(selected),
            domain_auroc_selected_space=dauc,
            effective_sample_size=float(ess),
    ...
    df = pd.DataFrame(rows)
    df.to_csv(
        out / f"invariant_density_weight__{target}.csv", index=False
    )
    if weight_frames:
        pd.concat(weight_frames, ignore_index=True).to_csv(
            out / f"invariant_density_weight_source_weights__{target}.csv",
            index=False
        )
    return df, peak_df
...
def main():
    ...
    ap.add_argument(
        "--analyses",
        default="temporal_site,ast_selection,support,conditional_shift,lineage,density_weight,invariant_peaks,invariant_peaks_target_blind,source_defined_clusters,invariant_density_weight,selective",
    ...
    if "all" in wanted:
        wanted = [
            ...
            "invariant_peaks", "invariant_peaks_target_blind",
            "source_defined_clusters", "invariant_density_weight",
            "selective",
        ]
    ...
    for name in wanted:
        ...
        try:
            ...
            elif name == "invariant_peaks_target_blind":
                df, peaks, selected = analysis_invariant_peaks_target_blind(
                    lab, mdir, root, args.species, args.drug, args.target, out,
                    topk=args.topk_peaks, seed=args.seed
                )
                print(df.to_string(index=False))
                print("\nTop target-blind selected bins:")
                show = peaks[peaks.selected].head(20)
                cols = ["bin", "mz_approx", "median_abs_smd", "selection_sites"]
                print(show[cols].to_string(index=False) if len(show) else "(none)")

            elif name == "source_defined_clusters":
                summary, comp, local = analysis_source_defined_clusters(
                    lab, mdir, root, args.species, args.drug, args.target, out,
                    ncomp=args.ncomp, n_clusters=args.clusters, seed=args.seed
                )
            ...
            elif name == "invariant_density_weight":
                df, peaks = analysis_invariant_density_weight(
                    lab, mdir, root, args.species, args.drug, args.target, out,
                    topk=args.topk_peaks, seed=args.seed
                )
                print(df.to_string(index=False))
